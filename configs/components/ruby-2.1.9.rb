component "ruby-2.1.9" do |pkg, settings, platform|
  ...
  if target_doubles.has_key?(settings[:platform_triple])
    rbconfig_topdir = File.join(settings[:ruby_dir], 'lib', 'ruby', '2.1.0', target_doubles[settings[:platform_triple]])
  else
  ...
  pkg.install do
    [
      ...
      "cp #{target_dir}/original_rbconfig.rb #{settings[:datadir]}/doc/rbconfig-2.1.9-orig.rb",
